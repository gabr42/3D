...
function g_rotate(angle, origin = [0, 0, 0], v = [0, 0, 1], points, _R = undef) = 
  ! is_vector(points)
    ...
    : let (pt = points)
      g_translate(origin, _R * g_translate(-origin, pt));
...
function g_twist(curve, angle, points, _full_len = undef) =
  ...
  : let (pt = points,
    ...
    g_rotate(angle * pt_len/_full_len, find[0], curve[find[1]+1] - curve[find[1]], pt);
...
function g_reflow(source_path, target_path, points, _source_len = undef) = 
  ...
  : let (pt = points,
         ...
         nearest_t = curve_find_offset(target_path, nearest_len/_source_len),
